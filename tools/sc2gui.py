#!/usr/bin/env python3
"""Reusable bridge to the StarCraft II editor running on the Windows host.

The editor is a Win32/#32770 app.  We drive it from this Linux box by shipping
PowerShell scripts over SSH (port 2222 lands in WSL, so Windows binaries must be
called through /mnt/c/...).  Every script is UTF-16LE + base64 encoded and handed
to powershell.exe -EncodedCommand.

Usage:
    python3 tools/sc2gui.py -c 'Write-Output "hello"'      # inline body
    python3 tools/sc2gui.py probes/probe_unit_dialog.ps1   # body from file

The PRELUDE below is injected in front of every body.  It declares the P/Invoke
surface plus a small helper vocabulary:

    $P, $HP, $REMOTE, $RTEXT      process / remote-memory handles
    Find-Main                     main editor window (#32770)
    Get-Trees                     @(listTree, elemTree) discovered by geometry
    Find-Win <titleRegex>         top-level window of the editor process
    Kids <hwnd>                   visible children:  "hwnd|class|text"
    KidsAll <hwnd>                all children:       "hwnd|class|visible|L,T,R,B|text"
    Get-RectStr <hwnd>            "L,T,R,B"
    Get-NodeText <tree> <hItem>   TVM_GETITEMW text
    Get-Root / Get-Next / Get-Child / Get-Sel   TVM_GETNEXTITEM wrappers
    Select-Node <tree> <hItem>    TVM_SELECTITEM
    Click-Node <tree> <hItem>     select + real WM_LBUTTONDOWN/UP (unlocks WM_COMMAND)
    Click-Btn <hwnd>              PostMessage BM_CLICK
    Read-ListItem <lv> <idx>      LVM_GETITEMTEXTW
    Set-ListSel <lv> <idx>        LVM_SETITEMSTATE (selected + focused)
    List-Count <lv>               LVM_GETITEMCOUNT
    Set-EditText <edit> <text>    WM_SETTEXT then per-char WM_CHAR (Blizzard ctrls)
"""
...
def run(body: str, timeout: int = 180, keep: bool = False) -> str:
    """Ship a PowerShell body to the Windows host and return its stdout.

    Scripts are written to a file and executed with -File: the -EncodedCommand
    route hits the 32K Windows command-line limit for anything non-trivial.
    """
    script = build(body)

    local = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "work", "last.ps1")
    local = os.path.abspath(local)
    os.makedirs(os.path.dirname(local), exist_ok=True)

    # UTF-8 with BOM: PowerShell 5.1 otherwise reads non-ASCII as ANSI.
    with open(local, "w", encoding="utf-8-sig", newline="\r\n") as fh:
        fh.write(script)

    remote = "/mnt/c/Users/Administrator/AppData/Local/Temp/dsh_sc2gui.ps1"
    subprocess.run(
        ["scp", "-q", "-P", PORT, local, f"{HOST}:{remote}"],
        check=True,
        timeout=60,
    )

    win_path = r"C:\Users\Administrator\AppData\Local\Temp\dsh_sc2gui.ps1"
    proc = subprocess.run(
        ["ssh", "-o", "ConnectTimeout=10", "-p", PORT, HOST,
         f'{PWSH} -NoProfile -ExecutionPolicy Bypass -File "{win_path}"'],
        capture_output=True,
        text=True,
        encoding="utf-8",
        errors="replace",  # PowerShell emits GBK error text on a zhCN host
        timeout=timeout,
    )
...
def main() -> int:
    ...
    ap.add_argument("file", nargs="?", help="PowerShell body file")
    ap.add_argument("-c", "--command", help="inline PowerShell body")
    ...
    sys.stdout.write(run(body, timeout=args.timeout))
    return 0
