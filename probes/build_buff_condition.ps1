# Give $TRIGGER a Comparison condition: (Used Dialog Item) == $VAR
...
function Get-SentenceButtons([IntPtr]$dlg) {
  ...
  foreach ($k in [SC2]::KidsAll($dlg)) {
    if ($k -match "\|Button\|") {
      ...
      if ([int]$r[1] -ge 575 -and [int]$r[1] -lt 620) { $res += ,@($h, (Get-WindowTitle $h)) }
    }
  }
  return $res
}
...
Write-Output "=== 打开 Configure Condition ==="
...
if (-not (Select-ListItemByText $dlg "Comparison" 147 149)) {
  Write-Output "找不到 Comparison"
  $cnt = List-Count $lv
  for ($i = 0; $i -lt [Math]::Min($cnt, 20); $i++) { Write-Output ("  [" + $i + "] " + (Read-ListItem $lv $i)) }
}
Start-Sleep -Milliseconds 1500

Write-Output "=== 左槽: Used Dialog Item ==="
$btns = Get-SentenceButtons $dlg
foreach ($x in $btns) { Write-Output ("  按钮 '" + $x[1] + "'") }
$left = $btns[0][0]
Click-Btn $left
...
if (Select-ListItemByText $d2 "Used Dialog Item" 43 42) {
  Write-Output "  已选 Used Dialog Item"
} else {
...
Start-Sleep -Milliseconds 2200
$d3 = Find-Dialog
Write-Output ("  右槽 dialog = '" + (Get-WindowTitle $d3) + "'")
