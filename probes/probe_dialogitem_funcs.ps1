# Dump every "Triggering" function in the Any Compare picker, plus "Used"/"Event"
# matches, to find the clicked-dialog-item accessor.
...
foreach ($term in @("Triggering", "Used", "Event", "Any")) {
  Set-EditText $find ""
  ...
  for ($i = 0; $i -lt $cnt; $i++) {
    $t = Read-ListItem $flv $i
    if ($t -match "Dialog|Used|Triggering Dialog") { Write-Output ("    [" + $i + "] " + $t) }
  }
}
