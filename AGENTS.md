...
- **加载页面正文有 800 字上限（编辑器口径 = UTF-8 字节减去 `<n/>` 标签）**：`LoadingScreen/TextBody`（包内 zhCN）超限时编辑器会显示「文本长度 1636 / 800」并可能静默截断。`tools/bh_meta.py` 的 `loading_body()` 现在**自带预算**（`LOADING_LIMIT=800` / `LOADING_RESERVE=40`，`loading_units()` 复刻编辑器口径）：说明行按「旧→新」传入，装不下就**从最旧的开始丢**，只保留最新若干条，打包时打印 `加载页面 757/800（保留最新 6 条，截去较早 12 条)`。完整日志仍在地图详情页（前言里已写明）。
