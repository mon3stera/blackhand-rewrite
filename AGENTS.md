...
- **`-achieve <玩家名> <索引>` 管理命令（`gt_Achieve_Func`，需 `gv_oP[玩家][1]`）是第二处名称链**（~60820，与列表链**不同的键**）：漏补分支 → 该命令播报里名字是 `null`（原图 67 号当时就缺）。索引过滤与列表同款；写入无上界校验（`-achieve x 99` 会越界写），给成就加索引时顺带知道这条路径可用于**实测播报/发光字**。

**存档兼容性（用户 2026-09-11 提问定案）**：`A2`（角色向）= 4 段×20 词、`A3`（通用）= **71 词**，都是**定长全量词表**——无论解锁与否每次都写满（18583–18596 / 18586 循环 0..70）。所以：**索引不越界 ⇒ 加多少成就都零兼容影响**（老档那些位本来就是 0）。一旦要**扩容**（>70 个通用成就、或角色号 >20）：必须先扩数组维度 + 扩四个写入上界 + 读取上界，**并给读取加空串保护**（`StringWord` 越界返回 `""`，`StringToInt("")` 会报触发器错误并**中断读档函数**，先例 = 18506 行的报错「无法从''的参数中获取'int'」；读档被打断 ⇒ 后面 Reports/Blacklist/胜场都不加载 ⇒ 看起来像清档）。另一种做法是**新开 key/section**（例如 `I/A5`；我们已不验签，无签名障碍）——好处是只在一处判空、老结构一字不动，但**重置/清理路径（58817 附近的清分、`gf_CDisassembleMainBank`、保存格）要同步清**，否则出现"重置了但新成就还在"。当前通用成就 0–67 已用、**68/69/70 空着**，另 17/34/62 也可用；5–8/42 被显示过滤排除。
