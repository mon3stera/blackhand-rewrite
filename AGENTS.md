...
**渐变色名称（shw131，用户要求）**：`<c val="上色-下色">` 支持上下渐变（如天选者 `FFFF99-FF8800`）。名称色等用户验收；若某界面不渲染渐变再回退单色。
