...
- **二改红线（无原作者授权）**：本图属未经授权二改，**严禁修改/删除原图硬编码 handle 管理员链的任何既有分支**（~56944 起的 87 个，只能**追加**自己的分支）；给 handle 加权限一律走白名单追加，不动原分支的权限位。若日后公开发布，必须先收回测试放开项：prefer 会员/积分门（65440/65481）、大厅电脑计入（56020）等 `c_bhSoloBuild` 相关旁路。
