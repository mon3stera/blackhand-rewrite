...
- **二改红线（无原作者授权）**：本图属未经授权二改，**严禁修改/删除原图硬编码 handle 管理员链的任何既有分支**（~56944 起的 87 个，只能**追加**自己的分支）；给 handle 加权限一律走白名单追加，不动原分支的权限位。若日后公开发布，必须先收回测试放开项：prefer 会员/积分门（65440/65481）、大厅电脑计入（56020）等 `c_bhSoloBuild` 相关旁路。另需**存档迁移说明**：bank 命名空间绑定发布作者，改版发布后玩家原存档（`key`、`MBank13` 两份 .SC2Bank，含积分/成就/模型解锁）留在原作者目录下；脚本无法跨命名空间读取（引擎沙箱），迁移 = 玩家把两个 bank 文件复制到改版作者目录，**且必须重签**：签名 = SHA1(authorID + playerID + bankName + Σ(按名排序的 section.name + Σ(按名排序的 key.name + "Value" + 类型名 + 值)))，authorID=目标目录（地图作者 toon）、playerID=玩家 toon，算法已破解并实测可复现引擎签名（重签工具见 `tools/bank_resign.py`）。
- **shw134（BankVerify 引擎侧失败→载入即清档）**：私有发布改版图中，引擎 `BankVerify` 在载入时对引擎自己写的文件也返回失败，且失败时 bank 内容被引擎整体清空（BankValueGet 全空）→ 原图逻辑把存档当无效档清空重写，形成每局清档死循环（用户实测：正确重签的档仍被清；被清档文件的签名与文件内容对不上——引擎写盘时签名输入与序列化内容不一致，疑似私有图/元数据上下文问题；原图在国服回归期也出过同类故障后修复）。处置：三处 `BankVerify` 门槛（QQ/gift/I，3676/3680/3684 附近）改为 `c_bhSoloBuild == true` 时**短路放行**（条件前置，BankVerify 调用本身不执行，避免引擎清空内容）；`BankOptionSet(c_bankOptionSignature, true)` 保留，文件照常带签名以兼容原图/工具；防篡改由地图级 Checker（Math=(points+1)×(points+3) 等）承担。公开发布收回时随其它 `c_bhSoloBuild` 旁路一并还原。会员白名单是 handle 硬编码，不受迁移影响。
