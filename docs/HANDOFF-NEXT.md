...
**待用户验证 boot2-shw61**（提交 `0e27511`，修 shw59/60 引入的编译错误：①gf_BHNotify 形参 sound→soundlink（SoundLink() 返回 soundlink）；②gf_ESShadowKill 补 auto9B99A3AA_g/_var 声明。**教训：往生成代码插 while 广播循环必须同时在函数声明区补自动变量；涉及 SoundLink 传参一律 soundlink**。DT 语音待听验：DarkTemplar_What 首选，备选 _Yes/_Ready/_Pissed）**）：
