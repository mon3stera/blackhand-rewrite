...
**待用户验证 boot2-shw53**（提交 `3137a03`，按用户建议 `-solo` 解耦角色自动填充：删除 gf_BHSoloFill，-solo 只放行人数校验并启用 prefer；虚拟玩家补满由 gt_Init2 的 c_bhSoloBuild 块负责（开局后 1..15 未在 gv_players 的槽全部补为「电脑N」，含名字/颜色/房屋），角色全部由自设手动配置。**注意：大厅里加电脑（lobby 电脑玩家）仍是更完整的测试方式，虚拟补位玩家没有游戏内单位/界面存在**）**）：
