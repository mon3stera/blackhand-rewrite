...
## 〇-A、2026-09-11 天选者收尾 + 「随机：天谴」预设（最新状态，以此为准）

**待用户验证 boot2-shw133**（md5 `5d7ca1b465a33f3bdca3fd54b38ddbe4`，已 scp 至 `D:\StarCraft II\Maps\Test\`）：

1. **天选者收尾（shw127-132 已部署）**：免死后再次被投 → `gf_TXResumeDay()` 恢复白天（跳过审判必须成对恢复，否则永久卡死）；审判台揭示改公屏广播（`gf_CBSystemMessage` 橙色，跟随 -magnify）；雷击死亡文案两条新变体；天选者名称渐变 `FFFFFF99-FFFF8800`（**必须 8 位 ARGB**，6 位=地图无法运行且无 ScriptError，shw131 事故）。
2. **「随机：天谴」预设（shw133，本次新增）**：菜单第 34 项 `GCZTNAME` → variant 33 → `gf_VTQOptions`。锁定型四子变体 A-D（15 人档），固定天选者(3,32)；A=狱警/探员/教父+黑手随机×3(不包括致命)，B=观察者/教父/龙头，C=教父+城镇随机×5+全体随机，D=医生+瘟疫散布者×2+城镇随机×5（瘟疫散布者=3/14）。规格 `work/presets/tianqian.json`，生成器产出；开局白字广播条件含 `"tq"`；锁定三件套含 33。文案键 `GCZTNAME/GCZTDESC/GCZTSUBA-D/GCZTBTN`（名称/子变体/按钮用天选者同款渐变）。
3. **生成器两处修复**：克隆尾不再多输出一个 `}`（曾致配平 -1）；按钮键规格化 `btn_key`；`ALIAS` 登记天选者=(3,32)、`SLOT_TIER` 补全体随机(4/1)=tier 7。回归：dashenpan --selfcheck OK，生成体与源码插入体逐字符一致。
4. 待实机验证：天谴四档阵容/锁定/白字广播；天选者免死再投不卡死；渐变名称渲染。

## 〇-B、2026-09-10 预设可用性修复（最新状态，以此为准）

**待用户验证 boot2-shw69**（提交 `6581b72`）：

1. **槽位过滤**：`gf_VLoadSaveSlot` 的加载校验 `lv_x[lv_a][0] > gv_townMax`（townMax=30）会把 31 号角色（观察者/影武者）清零——预设阵容列表显示空行、实际阵容也缺人（"已选的角色不足"根因）。已改为 `> 31`。
2. **预设可编辑**：目录浏览白名单（gt_OSMenus_Func 6 处）与选中映射白名单（gt_OSRoleSelect_Func 5 处）补 `14/15/16/30/31`——捕风捉影/烙印/中世纪等现在可浏览目录、看角色详情、添加/移除角色；???类变体（随机:血锈等子板子）仍锁定。添加按钮禁用集 `[1,4,8,9,10,11,12]` 本就不含 30/31，未动。
3. **烙印补全**：MenuChange 补 item 31 描述分支 + `BHYINDESC` 文案；修复选中烙印无说明、右下标签残留"红与蓝"。
4. 事故记录：shw67（`dfd6196`）补丁先改原型行再用失效偏移截函数尾 → 脚本读取失败 → 取名后卡死；shw68（`1e9b2f7`）重建并修复 `gv_bankRoleAchievements[x][1][31]` 越界（维度 int[16][9][21]）。
5. 注意：预设再次点「采纳设定」会用预设默认阵容覆盖手动修改（原图 fill 语义，未改）。
...
**待用户验证 boot2-shw67**（提交 `dfd6196`，新增预设「捕风捉影」：变体菜单第 32 项 → variantSelection=31 → gf_VBFCZOptions（克隆红与蓝模板）。15 人阵容=侦探/观察者/4×城镇随机/城镇调查/政府/保护/教父/陪侍/黑手随机/影武者(3/31)/女巫(3/4)/中立温和，14/13/12 人依次递减城镇随机，≤11 人沿用红与蓝的拒绝提示。**顺带修复烙印菜单项**：BHYIN01 名字键此前从未定义（菜单显示原始键名），现=「烙印」，且 gf_VE78399E58DB0（variant 30=菜单第31项=烙印入口）的确认按钮文案同步改为烙印——注意该函数内部的 panel 文案原本写着红与蓝（F9444261→BHYIN01），若用户发现烙印内容与其预期不符需再核对函数体。随机组槽位语义：4[2]=城镇随机/4[3]=黑手随机/4[4]=政府/4[5]=调查/4[6]=保护/4[14]=中立温和）**）：
- 旧记录（shw66，`2b5eba8`）：观察者 1/31 端到端；shw65：侦探类线索枚举；strings：shw 23 + gcz 23 条）：（提交 `2b5eba8`，新角色：城镇观察者 1/31，机制=监视者的"到访者"变体：单目标夜间观察、**可观察自己**，天亮私密播报造访目标者的**角色名清单**（如「党徒、医生、警长 访问了你的目标」），无人造访有专属提示；无案底（同监视者，不设罪名标记）；探员线索归侦探类（F2239D82 枚举已加观察者）。交互=开关式：点行选择（选中行变蓝）、再点同行取消、点自己行=观察自己。**顺带修复审查页中立列表缺失影武者**（上界 17→18、确认映射 >=18 加 12）。观察者实现细节（含审查页偏移映射算法：城镇上界 31+>=30 回退 1）见 AGENTS.md 端到端流程。自设面板城镇段现为 28 项（索引 28→角色31）**）：
- 旧记录（shw65，`81747f8`）：侦探类线索枚举加影武者；影武者本体用户已验收（DT 语音 DarkTemplar_What + Wait 2.6s）；strings：shw 23 + gcz 19 条）：（提交 `81747f8`，侦探类线索枚举加入影武者：覆盖共享键 `F2239D82`→「侦探、监视者、密探/先锋、纵火者、影武者」（1,15/1,16/1,30/2,10/3,5/3,31/5,10 七角色共用；sc2pack merge_strings 支持覆盖原键）。**用户确认影武者本体已无问题；DT 语音 DarkTemplar_What 已认可**）**）：
- 旧记录（shw64，`a4f3311`）：语音后 Wait 2.6s；shw63（`961eff2`）：语音先行；shw62（`d149eaf`）：死亡描述首杀模式；strings 23 条）：（提交见 git log，微调：DT 语音后 Wait 1.2s→2.6s，等语音播完再出刀声+红字；2.6s 若仍偏短/偏长直接改这个数）**）：
...
- 旧记录（shw59，`35f5419`）：出场流程+gf_BHNotify；shw57（`57997b5`）：死亡方式 w；shw55（`ceeaf78`）：阵营「无」；（提交 `35f5419`，影武者夜间击杀补全出场流程：台词 SHWQUOTE「我从阴影中现身」→ Zeratul 音效 → 全场红字 SHWHEAR 破风声 → 目标私息 SHWNIGHT；新增 gf_BHNotify（虚拟补位目标的私密消息改发主机，solo 可见；SHWNIGHT×2/SHWSURV/被救治提示 4 处走路由）。注：51592af 又只提交了 strings，代码在 35f5419；shw58 没有代码改动请勿使用）**）：
- 旧记录（shw57，`57997b5`）：死亡方式字母码 w+SHWDEATH1-3+SHWDM；shw55（`ceeaf78`）：阵营「无」；shw54（`2b431fa`）：虚拟补位时序；strings 共 24 条）：（提交 `57997b5`，影武者击杀补死亡方式：字母码 K→w、随机 3 选 1 死亡描述 SHWDEATH1-3、验尸官私密播报 SHWDM；strings 共 22 条。注：e57660e 只提交了 strings，代码在 57997b5）**）：
- 旧记录（shw55，提交 `ceeaf78`）：角色卡阵营「无」修复；shw54（`2b431fa`）：虚拟补位挪到 Init2 开头；shw53（`3137a03`）：-solo 解耦；shw52（`fb96148`）：几率/数量修复；（提交 `ceeaf78`，修复：对局内角色卡影武者阵营显示「（致命）」缺「无」——NeutralRoleText 的 (3,31) 分支 boxText[0] 只追加了 D7C97124，漏了 CE62498E「无」前缀；已与 TownRoleText 防御分支统一）**）：
- 旧记录（shw54，提交 `2b431fa`）：虚拟补位块挪到 Init2 开头；shw53（`3137a03`）：-solo 解耦角色填充；shw52（`fb96148`）：帮助卡片特性两行化+几率/数量修复；strings 共 18 条）：（提交 `2b431fa`，修复：c_bhSoloBuild 虚拟补位块原在 gt_Init2 末尾（Wait 2+3+5 秒后），发角色/夜间面板早已完成导致「没玩家」；已挪到 Init2 实现开头（主玩家循环之前），开局即补满 15 人，夜间行动面板每个存活其他玩家一行双目标按钮；「问罪」为用户取的影武者行动名）**）：
- 旧记录（shw53，提交 `3137a03`）：-solo 解耦角色填充；shw52（`fb96148`）：帮助卡片特性两行化+几率/数量修复；shw50（`c563399`）：拼音循环→31；shw49（`f7772e7`）：破风声+SHWSURV；strings 共 18 条）：（提交 `3137a03`，按用户建议 `-solo` 解耦角色自动填充：删除 gf_BHSoloFill，-solo 只放行人数校验并启用 prefer；虚拟玩家补满由 gt_Init2 的 c_bhSoloBuild 块负责（开局后 1..15 未在 gv_players 的槽全部补为「电脑N」，含名字/颜色/房屋），角色全部由自设手动配置。**注意：大厅里加电脑（lobby 电脑玩家）仍是更完整的测试方式，虚拟补位玩家没有游戏内单位/界面存在**）**）：
- 旧记录（shw52，提交 `fb96148`）：帮助面板卡片特性两行化 + 出现几率/预计数量修复（5 个循环上界→31）；shw51（`fdd94d3`）：[已废弃] BHSoloFill 补满 15 槽——shw53 起已删除；shw50（`c563399`）：拼音循环→31；shw49（`f7772e7`）：破风声+SHWSURV；strings 共 18 条）：（提交 `fb96148`，新增：①帮助面板影武者卡特性两行化；②出现几率/预计数量修复——`gf_OSComputeOptions` 里几率清零/计数/标志重置/归一化 5 个角色号循环上界 `gv_townMax`(30)→31，入配置后几率正确累计（此前 0%/1000））**）：
