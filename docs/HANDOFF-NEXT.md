...
**待用户验证 boot2-shw54**（提交 `2b431fa`，修复：c_bhSoloBuild 虚拟补位块原在 gt_Init2 末尾（Wait 2+3+5 秒后），发角色/夜间面板早已完成导致「没玩家」；已挪到 Init2 实现开头（主玩家循环之前），开局即补满 15 人，夜间行动面板每个存活其他玩家一行双目标按钮；「问罪」为用户取的影武者行动名）**）：
