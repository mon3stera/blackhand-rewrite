...
**待用户验证 boot2-shw66**（提交 `2b5eba8`，新角色：城镇观察者 1/31，机制=监视者的"到访者"变体：单目标夜间观察、**可观察自己**，天亮私密播报造访目标者的**角色名清单**（如「党徒、医生、警长 访问了你的目标」），无人造访有专属提示；无案底（同监视者，不设罪名标记）；探员线索归侦探类（F2239D82 枚举已加观察者）。交互=开关式：点行选择（选中行变蓝）、再点同行取消、点自己行=观察自己。**顺带修复审查页中立列表缺失影武者**（上界 17→18、确认映射 >=18 加 12）。观察者实现细节（含审查页偏移映射算法：城镇上界 31+>=30 回退 1）见 AGENTS.md 端到端流程。自设面板城镇段现为 28 项（索引 28→角色31）**）：
